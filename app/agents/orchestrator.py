...
from app.models.schemas import (
    DetailedAnalyzeResponse, HateSpeechClassification, PolicySummary,
    ActionRecommendation, ConfidenceLevel, ActionType, SeverityLevel
)
...
class HateSpeechOrchestrator:
    ...
    async def run(self, text: str) -> DetailedAnalyzeResponse:
        ...
        try:
            ...
            retrieval_result = await self.retriever._execute(original_text, classification)
            policies = retrieval_result.policies

            # Step 3: Reasoning using policies
            reasoning_output = await self.reasoner._execute(original_text, policies, classification)
            explanation = reasoning_output.get("explanation", "No global explanation returned.")
            ...
            for p in policies:
                p.explanation = policy_explanations.get(p.id, "No specific summary provided.")

            # Build structured response
            return self._build_detailed_response(classification, policies, explanation, recommendation)
    ...
    def _build_action_recommendation(self, classification) -> ActionRecommendation:
        ...
        mapping = {
            "hate": (ActionType.ESCALATE, SeverityLevel.HIGH, "Escalate due to serious hate speech."),
            "toxic": (ActionType.WARN, SeverityLevel.MEDIUM, "Warn user for toxic language."),
            "offensive": (ActionType.REVIEW, SeverityLevel.LOW, "Flag offensive content for review."),
            "ambiguous": (ActionType.REVIEW, SeverityLevel.LOW, "Ambiguous content—requires review."),
            "neutral": (ActionType.ALLOW, SeverityLevel.NONE, "Content is safe and can be allowed.")
        }
        action, severity, reasoning = mapping.get(label, (ActionType.ALLOW, SeverityLevel.NONE, "Content is safe."))
        return ActionRecommendation(action=action, severity=severity, reasoning=reasoning)
